...
Begin WebContainer conMemInfo
   ...
   Begin WebLabel Label32
      ...
      Style           =   "1157897102"
      TabOrder        =   73
      Text            =   "Enter Membership Number, press search. Press edit to change the information if needed."
      Top             =   14
      VerticalCenter  =   0
      Visible         =   True
      Width           =   606
      ...
      _VerticalPercent=   0.0
   End
   Begin WebTextField txtMembershipNumber
      AutoCapitalize  =   True
      AutoComplete    =   True
      AutoCorrect     =   True
      CueText         =   ""
...
#tag WindowCode
	#tag Event
		Sub Shown()
		  'RSA! Commented out for XoJo
		  
		  If Not CheckPermission() then
		    'MsgBox("I'm sorry your chapter does not have permission to use this feature.")
		  ...
		  txtMembershipNumber.ReadOnly = False
		  txtMembershipNumber.SetFocus
		End Sub
	#tag EndEvent
	...
	#tag Method, Flags = &h0
		Function CheckPermission() As Boolean
		  ...
		  lnPid = GetPID(Session.gnUid)
		  
		  lsSql = "Select ChapterCode from tblpeoplechapterofficers where PersonID = " + Str(lnPid)
		  ...
		  If rs.RecordCount = 0 then
		    MsgBox("You are not a chapter officer, please contact support@aspe.org" )
		    Return False
		  end
		  
		  Session.gsAuthorizedChapterCode = rs.Field("ChapterCode").StringValue
		  
		  lsSql = "Select ChapterCode, Allowreinstatements from tblaspechaptercodes where ChapterCode = '" + Session.gsAuthorizedChapterCode + "' "
		  lsSql = lsSql + "And Allowreinstatements = 1 "
		  ...
		  if rs.RecordCount = 0 then
		    Session.gsAuthorizedChapterCode = ""
		    MsgBox(" I'm sorry your chapter does not have authorization to use this service.")
	...
	#tag Method, Flags = &h0
		Sub LoadInfo(lsMemNum as String = "0", lbConfirmDlg as Boolean = True)
		  Dim rs as RecordSet
		  Dim lsSql as String
		  
		  
		  
		  lsSql = "Select * from tblPeople where MemberNumber = " + lsMemNum
		  
		  rs =Session.sesAspeDB.SQLSelect(lsSql)
		  
		  if IsNull(rs) then return
		  
		  if Session.sesAspeDB.CheckDBError then return
		  
		  If rs.RecordCount = 0 then
		    MsgBox("I'm sorry but ASPE does not have any membership record with that number." + EndOfLine + EndOfLine + _
		    "Please make sure you got the membership number correct."  + EndOfLine + EndOfLine + _
		    "If you need further assistance send an email to support@aspe.org.")
		    btnEdit.Enabled = False
		    Return
		  end
		  
		  if rs.Field("MemStatus").StringValue <> "Old Member" then
		    MsgBox("This person is not in old member. They must have renewed already.")
		    Return
		  end
		  
		  if rs.Field("MemStatus").StringValue = "Deceased" then
		    MsgBox("I'm sorry this person has passed away.")
		    Return
		  end
		  
		  if rs.Field("ChapterCode").StringValue <> Session.gsAuthorizedChapterCode then
		    MsgBox("I'm sorry this person is not in your chapter.")
		    Return
		  end
		  ...
		  if lbConfirmDlg then
		    frmAppllcation.dlgCorrectPersonYN.Show
		  end
	...
	#tag Method, Flags = &h0
		Sub ResetForm()
		  ...
		  chkASHRAE.Value = False
		  chkASSE.Value = False
		  chkNSPE.Value = False
		  chkIAPMO.Value = False
		  chkMCCA.Value = False
		  chkPCA.Value = False
		  chkNFPA.Value = False
		  chkICC.Value = False
	...
	#tag Method, Flags = &h0
		Function SaveMemInfo() As Boolean
		  ...
		  oSQL.AddFields "FirstName", "LastName", "MembershipNum",                     "emailAddress"
		  oSQL.AddValues txtFirst.Text, txtLast.Text, txtMembershipNumber.Text, txtPrimaryEmail.Text
		  ...
		  if Session.gnRecNo = 0 then
		    Session.gnRecNo = Session.sesWebDB.LastID("memapplications")
		  end
	...
	#tag Event
		Sub Action()
		  If me.Caption = "Edit" then
		    ...
		    btnSave.Enabled = False
		    LoadInfo(txtMembershipNumber.text, false)
		    SetReadOnly(Self, False)
		    me.Caption = "Edit"
		  end
...
#tag Events btnSave
	...
	#tag EndEvent
#tag EndEvents
#tag Events txtMembershipNumber
	#tag Event
		Sub TextChanged()
		  if me.Text = "" then
		    ResetForm
	...
	#tag EndEvent
#tag EndEvents
#tag Events btnSearchMemberNumber
	#tag Event
		Sub Action()
		  LoadInfo(txtMembershipNumber.text)
